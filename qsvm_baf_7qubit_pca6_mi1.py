#!/usr/bin/env python3
# =============================================================================
# QSVM 7 QUBIT — BAF NeurIPS 2022
# Pipeline: preprocessing -> MI -> PCA(6) + top-MI feature -> ZZFeatureMap(7 qubit)
# Obiettivo:
# 1. Valutare una configurazione intermedia tra 6 e 8 qubit
# 2. Usare 6 componenti PCA per aumentare la varianza spiegata
# 3. Conservare esplicitamente una feature supervisionata molto informativa
#    sul 7° qubit tramite Mutual Information
...
print("=" * 84)
print("QSVM 7 QUBIT — PCA(6) + TOP FEATURE MI")
print("=" * 84)
print("Questo script estende la pipeline precedente aggiungendo:")
print(" - PC6 come ulteriore componente principale rispetto al setup 6 qubit")
print(" - una top feature supervisionata da Mutual Information come 7° input")
print(" - una ZZFeatureMap con feature_dimension = 7")
...
# preservare esplicitamente sul 7° qubit.
...
print(f"\n[TOP MI] Settima feature selezionata: {top_mi_feature} (MI={top_mi_score:.6f})")
print("[INFO] Questa feature verrà aggiunta direttamente dopo PCA(6) come input del 7° qubit.")
...
ax.invert_yaxis()
plt.tight_layout()
plt.savefig("mi_scores_7q.png", dpi=150)
plt.close()
print("[PLOT] Salvato mi_scores_7q.png")

# -----------------------------------------------------------------------------
# BLOCCO 4 — Feature Projection: PCA a 6 componenti + 1 feature MI
# -----------------------------------------------------------------------------
print("\n" + "=" * 70)
print("BLOCCO 4 — FEATURE PROJECTION: PCA -> 6 COMPONENTI")
print("=" * 70)

# In questo setup usiamo:
# - 6 componenti PCA -> per aumentare la varianza spiegata rispetto alla versione a 6 qubit base
...
# Costruisci il vettore finale a 7 dimensioni:
# [PC1, PC2, PC3, PC4, PC5, PC6, top_mi_feature]
X_7 = np.hstack([X_pca, mi_feature_values])
feature_cols_7 = [f"PC{i}" for i in range(1, N_PCA + 1)] + [top_mi_feature]

print("Varianza spiegata dalle 6 componenti PCA:")
for i, (ev, cum) in enumerate(zip(explained, cumulative), start=1):
    print(f" PC{i}: {ev*100:.2f}% (cumulativo: {cum*100:.2f}%)")

print(f"\n[INFO] I primi 6 input sono PCA: PC1, PC2, PC3, PC4, PC5, PC6")
print(f"[INFO] Il 7° input è la feature supervisionata: {top_mi_feature}")
print(f"[INFO] PCA(6) spiega {cumulative[-1]*100:.2f}% della varianza totale")
print("[INFO] Il 7° qubit non aumenta la varianza PCA, ma preserva una feature altamente informativa")
...
ax.axhline(cumulative[-1] * 100, color="#636EFA", linestyle=":", lw=2,
           label=f"PCA(6) = {cumulative[-1]*100:.1f}%")
ax.set_ylabel("Varianza spiegata (%)")
ax.set_title(f"PCA(6) + top MI feature: {top_mi_feature}")
ax.legend()
plt.tight_layout()
plt.savefig("pca_plus_mi_7q.png", dpi=150)
plt.close()
print("[PLOT] Salvato pca_plus_mi_7q.png")
...
# Manteniamo lo stesso setup sperimentale dei file precedenti:
# 150 campioni per classe -> 300 totali, compatibili con un benchmark ancora gestibile
N_CLASS = 150

# Crea un DataFrame con le 7 feature finali e il target
df_7 = pd.DataFrame(X_7, columns=feature_cols_7)
df_7["fraud_bool"] = y.values

legit = df_7[df_7["fraud_bool"] == 0]
fraud = df_7[df_7["fraud_bool"] == 1]
...
X_bal = bal[feature_cols_7].values
...
print(f"Feature finali usate: {feature_cols_7}")
...
# Per questo motivo riscaliamo le 7 dimensioni finali nell'intervallo [-pi, pi].
...
# 8a. Crea una ZZFeatureMap a 7 qubit
...
# SVM lineare classica sullo stesso spazio finale a 7 dimensioni
...
ax.set_title(f"Kernel quantistico — 7 qubit\nTrain set ({X_train.shape[0]} campioni)")
plt.tight_layout()
plt.savefig("kernel_matrix_7q.png", dpi=150)
plt.close()
print("[PLOT] Salvato kernel_matrix_7q.png")
...
fig.suptitle("QSVM 7 qubit vs SVM classiche — PCA(6)+MI(1)", fontsize=14, fontweight="bold")
plt.tight_layout()
plt.savefig("benchmark_metrics_7q.png", dpi=150)
plt.close()
print("[PLOT] Salvato benchmark_metrics_7q.png")
...
fig.suptitle("Confusion matrices — setup 7 qubit", fontsize=13)
plt.tight_layout()
plt.savefig("confusion_matrices_7q.png", dpi=150)
plt.close()
print("[PLOT] Salvato confusion_matrices_7q.png")
...
# il vero classificatore lavora nello spazio completo a 7 dimensioni e usa il
...
# a 7 dimensioni, ma è utile come illustrazione qualitativa.
...
fig.suptitle("Decision Boundaries — PC1 vs PC2 (Training Set, setup 7 qubit)",
             fontsize=14, fontweight="bold")
plt.tight_layout()
plt.savefig("decision_boundaries_7q.png", dpi=150)
plt.close()
print("[PLOT] Salvato decision_boundaries_7q.png")
...
df_res.to_csv("benchmark_results_7q.csv")
print("[SAVE] benchmark_results_7q.csv")
np.save("K_train_7q.npy", K_train)
np.save("K_test_7q.npy", K_test)
print("[SAVE] K_train_7q.npy, K_test_7q.npy")
np.save("X_train_7q.npy", X_train)
np.save("X_test_7q.npy", X_test)
np.save("y_train_7q.npy", y_train)
np.save("y_test_7q.npy", y_test)
print("[SAVE] X_train_7q.npy, X_test_7q.npy, y_train_7q.npy, y_test_7q.npy")

with open("readme_7q.txt", "w", encoding="utf-8") as f:
    f.write(
        "QSVM 7 qubit\n"
        f"Input finale: PC1, PC2, PC3, PC4, PC5, PC6, {top_mi_feature}\n"
        f"Top feature MI: {top_mi_feature} (score={top_mi_score:.6f})\n"
        f"Varianza cumulativa PCA(6): {cumulative[-1]*100:.2f}%\n"
        "Nota: il passaggio a PCA(6) aumenta la varianza spiegata rispetto al setup con meno componenti.\n"
        "Il 7° qubit non aumenta la varianza PCA, ma preserva informazione supervisionata utile.\n"
...
print("[SAVE] readme_7q.txt")

print("\n" + "=" * 84)
print("PIPELINE 7 QUBIT COMPLETATA")
print("=" * 84)
print("OUTPUT GENERATI:")
print("- mi_scores_7q.png")
print("- pca_plus_mi_7q.png")
print("- zzfeaturemap_7q.txt")
print("- kernel_matrix_7q.png")
print("- benchmark_metrics_7q.png")
print("- confusion_matrices_7q.png")
print("- decision_boundaries_7q.png")
print("- benchmark_results_7q.csv")
print("- K_train_7q.npy / K_test_7q.npy")
print("- X_train_7q.npy / X_test_7q.npy / y_train_7q.npy / y_test_7q.npy")
print("- readme_7q.txt")
print("=" * 84)
