...
# -----------------------------------------------------------------------------
# BLOCCO 10a — Visualizzazione kernel matrix migliorata
# Heatmap ordinata per classe + istogramma valori off-diagonal
# -----------------------------------------------------------------------------
print("\n" + "=" * 70)
print("BLOCCO 10a — KERNEL MATRIX: HEATMAP ORDINATA + OFF-DIAGONAL HIST")
print("=" * 70)

# Ordina il train set per classe, così la heatmap mostra eventuali blocchi
order = np.argsort(y_train)
K_ord = K_train[order][:, order]
y_ord = np.array(y_train)[order]

# Maschera per escludere la diagonale
diag_mask = np.eye(K_ord.shape[0], dtype=bool)
off_diag_vals = K_ord[~diag_mask]

print(f"[KERNEL] Shape ordinata: {K_ord.shape}")
print(f"[KERNEL] Numero valori off-diagonal: {len(off_diag_vals)}")
print(f"[KERNEL] Off-diagonal mean = {off_diag_vals.mean():.6f}")
print(f"[KERNEL] Off-diagonal std  = {off_diag_vals.std():.6f}")
print(f"[KERNEL] Off-diagonal min  = {off_diag_vals.min():.6f}")
print(f"[KERNEL] Off-diagonal max  = {off_diag_vals.max():.6f}")

# Indici del punto di separazione tra classi per tracciare linee guida
class_changes = np.where(np.diff(y_ord) != 0)[0]
split_lines = class_changes + 1

# Figura con 3 pannelli:
# 1) heatmap ordinata completa
# 2) heatmap ordinata senza diagonale
# 3) istogramma dei valori off-diagonal
fig, axes = plt.subplots(1, 3, figsize=(20, 6))

# -------------------------------------------------------------------------
# PANNELLO 1 — Heatmap ordinata per classe
# -------------------------------------------------------------------------
sns.heatmap(
    K_ord,
    cmap="YlGnBu",
...
axes[0].set_title("Kernel matrix ordinata per classe", fontsize=12, fontweight="bold")
axes[0].set_xlabel("Campioni ordinati")
axes[0].set_ylabel("Campioni ordinati")

# Disegna linee di separazione tra classi
for s in split_lines:
    axes[0].axhline(s, color="red", lw=1.2, alpha=0.9)
    axes[0].axvline(s, color="red", lw=1.2, alpha=0.9)

# -------------------------------------------------------------------------
# PANNELLO 2 — Heatmap senza diagonale
# La diagonale è quasi sempre molto alta, quindi mascherarla aiuta a vedere
# meglio se i valori off-diagonal sono tutti simili tra loro
# -------------------------------------------------------------------------
sns.heatmap(
    K_ord,
    mask=diag_mask,
    cmap="magma",
    ax=axes[1],
    cbar_kws={"label": "Off-diagonal K(xi, xj)"},
    xticklabels=False,
    yticklabels=False,
    vmin=np.percentile(off_diag_vals, 1),
    vmax=np.percentile(off_diag_vals, 99)
)
axes[1].set_title("Kernel matrix senza diagonale", fontsize=12, fontweight="bold")
axes[1].set_xlabel("Campioni ordinati")
axes[1].set_ylabel("Campioni ordinati")

for s in split_lines:
    axes[1].axhline(s, color="cyan", lw=1.2, alpha=0.9)
    axes[1].axvline(s, color="cyan", lw=1.2, alpha=0.9)

# -------------------------------------------------------------------------
# PANNELLO 3 — Istogramma dei valori off-diagonal
# Se l'istogramma è molto stretto, può esserci kernel concentration
# -------------------------------------------------------------------------
sns.histplot(
    off_diag_vals,
    bins=35,
...
axes[2].axvline(off_diag_vals.mean(), color="red", linestyle="--", lw=2,
                label=f"mean = {off_diag_vals.mean():.3f}")
axes[2].axvline(off_diag_vals.mean() + off_diag_vals.std(), color="orange",
                linestyle=":", lw=2, label=f"+1 std = {off_diag_vals.mean() + off_diag_vals.std():.3f}")
axes[2].axvline(off_diag_vals.mean() - off_diag_vals.std(), color="orange",
                linestyle=":", lw=2, label=f"-1 std = {off_diag_vals.mean() - off_diag_vals.std():.3f}")
axes[2].set_title("Distribuzione valori off-diagonal", fontsize=12, fontweight="bold")
axes[2].set_xlabel("Valore kernel")
axes[2].set_ylabel("Frequenza")
axes[2].legend(fontsize=9)

fig.suptitle("Diagnostica kernel matrix — ordinamento per classe e concentrazione",
             fontsize=14, fontweight="bold")
plt.tight_layout()
plt.savefig("kernel_concentration_analysis_6q.png", dpi=150)
plt.close()

print("[PLOT] Salvato kernel_concentration_analysis_6q.png")
