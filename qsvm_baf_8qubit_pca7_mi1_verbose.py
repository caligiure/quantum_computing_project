#!/usr/bin/env python3
# =============================================================================
# QSVM 8 QUBIT — BAF NeurIPS 2022
# Pipeline: preprocessing -> MI -> PCA(7) + top-MI feature -> ZZFeatureMap(8 qubit)
# Obiettivo:
# 1. Aumentare ulteriormente la varianza spiegata usando 7 componenti PCA
# 2. Conservare esplicitamente una feature supervisionata molto informativa
#    sull'8° qubit tramite Mutual Information
...
print("=" * 84)
print("QSVM 8 QUBIT — PCA(7) + TOP FEATURE MI")
print("=" * 84)
print("Questo script estende la pipeline precedente aggiungendo:")
print(" - PC6 e PC7 come ulteriori componenti principali")
print(" - una top feature supervisionata da Mutual Information come 8° input")
print(" - una ZZFeatureMap con feature_dimension = 8")
...
# preservare esplicitamente sull'8° qubit.
...
print(f"\n[TOP MI] Ottava feature selezionata: {top_mi_feature} (MI={top_mi_score:.6f})")
print("[INFO] Questa feature verrà aggiunta direttamente dopo PCA(7) come input dell'8° qubit.")
...
ax.invert_yaxis()
plt.tight_layout()
plt.savefig("mi_scores_8q.png", dpi=150)
plt.close()
print("[PLOT] Salvato mi_scores_8q.png")

# -----------------------------------------------------------------------------
# BLOCCO 4 — Feature Projection: PCA a 7 componenti + 1 feature MI
# -----------------------------------------------------------------------------
print("\n" + "=" * 70)
print("BLOCCO 4 — FEATURE PROJECTION: PCA -> 7 COMPONENTI")
print("=" * 70)

# In questo setup usiamo:
# - 7 componenti PCA -> per aumentare la varianza spiegata rispetto alle versioni precedenti
...
# Costruisci il vettore finale a 8 dimensioni:
# [PC1, PC2, PC3, PC4, PC5, PC6, PC7, top_mi_feature]
X_8 = np.hstack([X_pca, mi_feature_values])
feature_cols_8 = [f"PC{i}" for i in range(1, N_PCA + 1)] + [top_mi_feature]

print("Varianza spiegata dalle 7 componenti PCA:")
for i, (ev, cum) in enumerate(zip(explained, cumulative), start=1):
    print(f" PC{i}: {ev*100:.2f}% (cumulativo: {cum*100:.2f}%)")

print(f"\n[INFO] I primi 7 input sono PCA: PC1, PC2, PC3, PC4, PC5, PC6, PC7")
print(f"[INFO] L'8° input è la feature supervisionata: {top_mi_feature}")
print(f"[INFO] PCA(7) spiega {cumulative[-1]*100:.2f}% della varianza totale")
print("[INFO] L'8° qubit non aumenta la varianza PCA, ma preserva una feature altamente informativa")
...
ax.axhline(cumulative[-1] * 100, color="#636EFA", linestyle=":", lw=2,
           label=f"PCA(7) = {cumulative[-1]*100:.1f}%")
ax.set_ylabel("Varianza spiegata (%)")
ax.set_title(f"PCA(7) + top MI feature: {top_mi_feature}")
ax.legend()
plt.tight_layout()
plt.savefig("pca_plus_mi_8q.png", dpi=150)
plt.close()
print("[PLOT] Salvato pca_plus_mi_8q.png")
...
# Crea un DataFrame con le 8 feature finali e il target
df_8 = pd.DataFrame(X_8, columns=feature_cols_8)
df_8["fraud_bool"] = y.values

legit = df_8[df_8["fraud_bool"] == 0]
fraud = df_8[df_8["fraud_bool"] == 1]
...
X_bal = bal[feature_cols_8].values
...
print(f"Feature finali usate: {feature_cols_8}")
...
# Per questo motivo riscaliamo le 8 dimensioni finali nell'intervallo [-pi, pi].
...
# 8a. Crea una ZZFeatureMap a 8 qubit
...
# SVM lineare classica sullo stesso spazio finale a 8 dimensioni
...
ax.set_title(f"Kernel quantistico — 8 qubit\nTrain set ({X_train.shape[0]} campioni)")
plt.tight_layout()
plt.savefig("kernel_matrix_8q.png", dpi=150)
plt.close()
print("[PLOT] Salvato kernel_matrix_8q.png")
...
fig.suptitle("QSVM 8 qubit vs SVM classiche — PCA(7)+MI(1)", fontsize=14, fontweight="bold")
plt.tight_layout()
plt.savefig("benchmark_metrics_8q.png", dpi=150)
plt.close()
print("[PLOT] Salvato benchmark_metrics_8q.png")
...
fig.suptitle("Confusion matrices — setup 8 qubit", fontsize=13)
plt.tight_layout()
plt.savefig("confusion_matrices_8q.png", dpi=150)
plt.close()
print("[PLOT] Salvato confusion_matrices_8q.png")
...
# il vero classificatore lavora nello spazio completo a 8 dimensioni e usa il
...
# a 8 dimensioni, ma è utile come illustrazione qualitativa.
...
fig.suptitle("Decision Boundaries — PC1 vs PC2 (Training Set, setup 8 qubit)",
             fontsize=14, fontweight="bold")
plt.tight_layout()
plt.savefig("decision_boundaries_8q.png", dpi=150)
plt.close()
print("[PLOT] Salvato decision_boundaries_8q.png")
...
df_res.to_csv("benchmark_results_8q.csv")
print("[SAVE] benchmark_results_8q.csv")
np.save("K_train_8q.npy", K_train)
np.save("K_test_8q.npy", K_test)
print("[SAVE] K_train_8q.npy, K_test_8q.npy")
np.save("X_train_8q.npy", X_train)
np.save("X_test_8q.npy", X_test)
np.save("y_train_8q.npy", y_train)
np.save("y_test_8q.npy", y_test)
print("[SAVE] X_train_8q.npy, X_test_8q.npy, y_train_8q.npy, y_test_8q.npy")

with open("readme_8q.txt", "w", encoding="utf-8") as f:
    f.write(
        "QSVM 8 qubit\n"
        f"Input finale: PC1, PC2, PC3, PC4, PC5, PC6, PC7, {top_mi_feature}\n"
        f"Top feature MI: {top_mi_feature} (score={top_mi_score:.6f})\n"
        f"Varianza cumulativa PCA(7): {cumulative[-1]*100:.2f}%\n"
        "Nota: il passaggio a PCA(7) aumenta la varianza spiegata rispetto alle versioni precedenti.\n"
        "L'8° qubit non aumenta la varianza PCA, ma preserva informazione supervisionata utile.\n"
...
print("[SAVE] readme_8q.txt")

print("\n" + "=" * 84)
print("PIPELINE 8 QUBIT COMPLETATA")
print("=" * 84)
print("OUTPUT GENERATI:")
print("- mi_scores_8q.png")
print("- pca_plus_mi_8q.png")
print("- zzfeaturemap_8q.txt")
print("- kernel_matrix_8q.png")
print("- benchmark_metrics_8q.png")
print("- confusion_matrices_8q.png")
print("- decision_boundaries_8q.png")
print("- benchmark_results_8q.csv")
print("- K_train_8q.npy / K_test_8q.npy")
print("- X_train_8q.npy / X_test_8q.npy / y_train_8q.npy / y_test_8q.npy")
print("- readme_8q.txt")
print("=" * 84)
