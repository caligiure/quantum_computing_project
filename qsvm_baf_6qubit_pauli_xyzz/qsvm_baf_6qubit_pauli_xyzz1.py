...
# Pipeline: preprocessing -> MI -> PCA(5) + top-MI feature -> PauliFeatureMap(6 qubit)
# Obiettivo:
...
# 3. Sostituire ZZFeatureMap con PauliFeatureMap(['X','Y','ZZ'], reps=1):
#    - 'X' : rotazione sull'asse X -> encoding in ampiezza, non solo fase
#    - 'Y' : rotazione sull'asse Y -> copre entrambi gli assi della sfera di Bloch
#    - 'ZZ': entanglement in fase tra coppie -> cattura correlazioni tra feature
#    Il risultato è un kernel quantistico geometricamente più ricco rispetto al
#    solo encoding di fase di ZZFeatureMap.
# 4. Mantenere lo stesso stile del file precedente: commenti completi, print
...
print("=" * 84)
print("QSVM 6 QUBIT — PauliFeatureMap ['X','Y','ZZ'] — PCA(5) + TOP FEATURE MI")
print("=" * 84)
print("Questo script estende la pipeline 6-qubit ZZFeatureMap sostituendo:")
print(" - ZZFeatureMap(reps=1) con PauliFeatureMap(paulis=['X','Y','ZZ'], reps=1)")
print(" - L'encoding ora copre sia l'asse X che Y della sfera di Bloch (ampiezza)")
print(" - Il termine ZZ mantiene l'entanglement in fase tra coppie di qubit")
print(" - PC1..PC5 come componenti principali, una top feature MI come 6° input")
...
ax.invert_yaxis()
plt.tight_layout()
plt.savefig("mi_scores_6q_pauli.png", dpi=150)
plt.close()
print("[PLOT] Salvato mi_scores_6q_pauli.png")
...
ax.legend()
plt.tight_layout()
plt.savefig("pca_plus_mi_6q_pauli.png", dpi=150)
plt.close()
print("[PLOT] Salvato pca_plus_mi_6q_pauli.png")
...
# -----------------------------------------------------------------------------
# BLOCCO 6 — Scaling angolare [-pi, pi] per PauliFeatureMap
# -----------------------------------------------------------------------------
print("\n" + "=" * 70)
print("BLOCCO 6 — SCALING ANGOLARE [-π, π] per PAULIFEATUREMAP")
print("=" * 70)

# PauliFeatureMap interpreta gli input come angoli di rotazione sulla sfera di Bloch.
# Il range [-pi, pi] è compatibile con le porte Rx, Ry e i termini ZZ:
#   - Rx(2*phi): range [-2pi, 2pi] -> ampiezza codificata sull'asse X
#   - Ry(2*phi): range [-2pi, 2pi] -> ampiezza codificata sull'asse Y
#   - ZZ: Rz(2*(pi-phi_i)*(pi-phi_j)) -> interazione in fase tra coppie
# Riscaliamo le 6 dimensioni finali nell'intervallo [-pi, pi].
angle_scaler = MinMaxScaler(feature_range=(-np.pi, np.pi))
X_angle = angle_scaler.fit_transform(X_bal)
print(f"Range finale input PauliFeatureMap: [{X_angle.min():.4f}, {X_angle.max():.4f}] rad")
...
# -----------------------------------------------------------------------------
print("\n" + "=" * 70)
print("BLOCCO 8 — QUANTUM KERNEL: PauliFeatureMap ['X','Y','ZZ'] + Fidelity-like compute-uncompute")
print("=" * 70)
...
# 8a. Crea una PauliFeatureMap a 6 qubit con paulis=['X','Y','ZZ']
#
# Differenza rispetto a ZZFeatureMap:
#   ZZFeatureMap usa solo ['Z','ZZ'], encodando esclusivamente fasi nella rotazione Rz.
#   PauliFeatureMap con ['X','Y','ZZ'] aggiunge:
#     - 'X' -> Rx(2*phi_i): rotazione sull'asse X della sfera di Bloch (encoding in ampiezza)
#     - 'Y' -> Ry(2*phi_i): rotazione sull'asse Y della sfera di Bloch (encoding misto)
#     - 'ZZ'-> Rz(2*(pi-phi_i)*(pi-phi_j)): entanglement in fase tra coppie di qubit
#
# reps=1: una singola ripetizione del layer (profondità contenuta)
# entanglement='full': tutte le coppie di qubit possono interagire tra loro
...
print("PauliFeatureMap 6-qubit creata (paulis=['X','Y','ZZ'], reps=1, entanglement='full').")
feature_map_txt = feature_map.decompose().draw(output="text", fold=-1)
feature_map_txt = str(feature_map_txt)
print(feature_map_txt)
with open("paulifeaturemap_6q.txt", "w", encoding="utf-8") as f:
    f.write(feature_map_txt)
print("[SAVE] Salvato paulifeaturemap_6q.txt")
...
ax.set_title(f"Kernel quantistico — 6 qubit PauliMap\nTrain set ({X_train.shape[0]} campioni)")
plt.tight_layout()
plt.savefig("kernel_matrix_6q_pauli.png", dpi=150)
plt.close()
print("[PLOT] Salvato kernel_matrix_6q_pauli.png")
...
fig.suptitle("QSVM 6 qubit vs SVM classiche — PauliMap(X,Y,ZZ) + PCA(5)+MI(1)", fontsize=14, fontweight="bold")
plt.tight_layout()
plt.savefig("benchmark_metrics_6q_pauli.png", dpi=150)
plt.close()
print("[PLOT] Salvato benchmark_metrics_6q_pauli.png")

# 10c. Confusion matrices
fig, axes = plt.subplots(1, 3, figsize=(15, 4))
models = [
    ("QSVM 6q PauliMap", y_pred_q),
    ("SVM Lineare", yp_l),
    ("SVM RBF", yp_r)
]
...
fig.suptitle("Confusion matrices — setup 6 qubit PauliMap", fontsize=13)
plt.tight_layout()
plt.savefig("confusion_matrices_6q_pauli.png", dpi=150)
plt.close()
print("[PLOT] Salvato confusion_matrices_6q_pauli.png")
...
fig.suptitle("Decision Boundaries — PC1 vs PC2 (Training Set, setup 6 qubit PauliMap)",
             fontsize=14, fontweight="bold")
plt.tight_layout()
plt.savefig("decision_boundaries_6q_pauli.png", dpi=150)
plt.close()
print("[PLOT] Salvato decision_boundaries_6q_pauli.png")
...
df_res.to_csv("benchmark_results_6q_pauli.csv")
print("[SAVE] benchmark_results_6q_pauli.csv")
np.save("K_train_6q_pauli.npy", K_train)
np.save("K_test_6q_pauli.npy", K_test)
print("[SAVE] K_train_6q_pauli.npy, K_test_6q_pauli.npy")
np.save("X_train_6q_pauli.npy", X_train)
np.save("X_test_6q_pauli.npy", X_test)
np.save("y_train_6q_pauli.npy", y_train)
np.save("y_test_6q_pauli.npy", y_test)
print("[SAVE] X_train_6q_pauli.npy, X_test_6q_pauli.npy, y_train_6q_pauli.npy, y_test_6q_pauli.npy")

with open("readme_6q_pauli.txt", "w", encoding="utf-8") as f:
    f.write(
...
print("[SAVE] readme_6q_pauli.txt")

print("\n" + "=" * 84)
print("PIPELINE 6 QUBIT — PauliFeatureMap COMPLETATA")
print("=" * 84)
print("OUTPUT GENERATI:")
print("- mi_scores_6q_pauli.png")
print("- pca_plus_mi_6q_pauli.png")
print("- paulifeaturemap_6q.txt")
print("- kernel_matrix_6q_pauli.png")
print("- benchmark_metrics_6q_pauli.png")
print("- confusion_matrices_6q_pauli.png")
print("- benchmark_results_6q_pauli.csv")
print("- K_train_6q_pauli.npy / K_test_6q_pauli.npy")
print("- X_train_6q_pauli.npy / X_test_6q_pauli.npy / y_train_6q_pauli.npy / y_test_6q_pauli.npy")
print("- readme_6q_pauli.txt")
print("=" * 84)
