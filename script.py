...
CAT = ["payment_type", "source", "device_os", "employment_status"]
for col in CAT:
    le = LabelEncoder()
    df[col] = le.fit_transform(df[col].astype(str))
print(f"[ENCODE] LabelEncoded: {CAT}")
...
with open("qsvm_baf_pipeline.py", "w") as f:
    f.write(code)
print("Codice scritto in qsvm_baf_pipeline.py")
print(f"Righe totali: {code.count(chr(10))}")