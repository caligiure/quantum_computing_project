...
# Pipeline: preprocessing -> MI -> PCA(5) + top-MI feature -> ZZFeatureMap(6 qubit) -> PQK
# Obiettivo:
...
print(" - kernel Proiettato (PQK): expectation values locali <Z_i> invece della fidelity globale")
...
# -----------------------------------------------------------------------------
# BLOCCO 6 — Scaling angolare [-pi, pi] per ZZFeatureMap (base del PQK)
# -----------------------------------------------------------------------------
print("\n" + "=" * 70)
...
# Questo scaling è mantenuto invariato anche per il PQK, che usa la stessa feature map.
...
# -----------------------------------------------------------------------------
print("\n" + "=" * 70)
print("BLOCCO 8 — QUANTUM KERNEL: ZZFeatureMap + Projected Quantum Kernel (PQK)")
print("=" * 70)
...
print("ZZFeatureMap 6-qubit creata (usata come encoder per il PQK).")
...
N_SHOTS = 8192  # shots per stima degli expectation values
print(f"[CONFIG] Shots per campione: {N_SHOTS}")

# Parametro gamma del kernel RBF applicato alle proiezioni PQK.
# Il PQK costruisce un kernel classico (RBF) sulle proiezioni quantistiche:
#   K_PQK(xi, xj) = exp(-gamma * ||proj(xi) - proj(xj)||^2)
# Il valore di default gamma=1.0 può essere ottimizzato tramite cross-validation.
PQK_GAMMA = 1.0
print(f"[CONFIG] PQK gamma (kernel RBF sulle proiezioni): {PQK_GAMMA}")


def compute_pqk_projection(x, fmap, backend, shots=N_SHOTS):
    """
    Calcola la proiezione PQK di un campione x.

    Il Projected Quantum Kernel (PQK) evita il problema della spectral flatness
    del kernel di fidelity globale misurando gli expectation values locali
    <Z_i> per ogni qubit i=0..n-1 dopo l'encoding del dato.

    Passi:
    1. Codifica x nel circuito U(x) tramite la ZZFeatureMap
    2. Misura ogni qubit nella base Z con N_SHOTS colpi
    3. Stima <Z_i> = P(0_i) - P(1_i) per ogni qubit i
    4. Restituisce il vettore di proiezione proj(x) = [<Z_0>, ..., <Z_{n-1}>]

    Questo vettore viene poi usato per costruire un kernel RBF classico,
    che è meno soggetto alla spectral flatness rispetto alla fidelity globale.
    """
    params = fmap.parameters
    circ = fmap.assign_parameters(dict(zip(params, x)))
    ...
    # Calcola P(0) per ogni qubit i e stima <Z_i> = P(0_i) - P(1_i)
    # Le bitstring in Qiskit sono in ordine little-endian: bit 0 = qubit 0
    exp_vals = np.zeros(n)
    for bitstring, count in counts.items():
        prob = count / shots
        for i, bit in enumerate(reversed(bitstring)):  # reversed: LSB = qubit 0
            if bit == "0":
                exp_vals[i] += prob   # contributo a P(0_i)
            else:
                exp_vals[i] -= prob   # contributo a -P(1_i)
    # exp_vals[i] = P(0_i) - P(1_i) = <Z_i>, in [-1, +1]
    return exp_vals


def compute_projections(X, fmap, backend, shots=N_SHOTS, label=""):
    """
    Calcola la matrice delle proiezioni PQK per tutti i campioni in X.
    Restituisce una matrice di shape (n_samples, n_qubits).
    """
    n_samples = len(X)
    n_qubits = fmap.num_qubits
    projections = np.zeros((n_samples, n_qubits))
    for idx, x in enumerate(X):
        if idx % 20 == 0:
            print(f" Proiezioni {label}: {idx}/{n_samples} ({idx/n_samples*100:.1f}%)", end="\r", flush=True)
        projections[idx] = compute_pqk_projection(x, fmap, backend, shots)
    print(f" Proiezioni {label}: {n_samples}/{n_samples} (100.0%)")
    return projections


def compute_kernel_entry(xi, xj, fmap, backend, shots=N_SHOTS):
    """
    [NON USATA NEL PQK — mantenuta per compatibilità con l'architettura del file]
    ...
    Nel PQK questa funzione è sostituita da compute_pqk_projection + RBF classico.
    """
    params = fmap.parameters
...
def compute_kernel_matrix(X_a, X_b, fmap, backend, shots=N_SHOTS, symmetric=False):
    """
    Calcola la matrice kernel PQK K[i,j] = exp(-gamma * ||proj(X_a[i]) - proj(X_b[j])||^2).

    A differenza del kernel di fidelity globale (compute-uncompute), il PQK:
    - Calcola le proiezioni proj(x) una volta sola per ogni campione (non a coppie)
    - Costruisce il kernel come RBF classico sulle proiezioni quantistiche
    - Riduce il numero di circuiti da O(n^2) a O(n), abbattendo il costo computazionale
    - È meno soggetto al collasso verso la matrice identità (spectral flatness)

    Se symmetric=True, le proiezioni di X_a vengono calcolate una volta sola
    e riutilizzate per entrambi gli indici i e j.
    """
    from sklearn.metrics.pairwise import rbf_kernel as sklearn_rbf_kernel

    if symmetric:
        # Stesso insieme di campioni: calcola le proiezioni una sola volta
        print(" [PQK] Calcolo proiezioni train...")
        proj_a = compute_projections(X_a, fmap, backend, shots, label="train")
        proj_b = proj_a
        compute_kernel_matrix._proj_train = proj_a  # salva per riuso in K_test
    else:
        # Insiemi diversi (test vs train): calcola proiezioni per X_a (test)
        print(" [PQK] Calcolo proiezioni test...")
        proj_a = compute_projections(X_a, fmap, backend, shots, label="test")
        # Riutilizza le proiezioni train già calcolate durante K_train.
        # NON ricalcolare: le misurazioni quantistiche sono stocastiche,
        # ricalcolare produce proiezioni diverse e rende K_train e K_test incoerenti.
        proj_b = compute_kernel_matrix._proj_train

    # Kernel RBF classico applicato alle proiezioni quantistiche
    # K[i,j] = exp(-gamma * ||proj_a[i] - proj_b[j]||^2)
    K = sklearn_rbf_kernel(proj_a, proj_b, gamma=PQK_GAMMA)
    return K

# 8c. Calcolo K_train
# Nel PQK il costo è O(n_train) circuiti invece di O(n_train^2/2)
print(f"\nCalcolo kernel matrix train ({X_train.shape[0]}x{X_train.shape[0]})...")
print(f" N° circuiti necessari (PQK): {X_train.shape[0]} (vs {X_train.shape[0] * (X_train.shape[0] + 1) // 2} per fidelity globale)")
print(f" Shots per campione: {N_SHOTS}")
...
fig.suptitle("QSVM PQK 6 qubit vs SVM classiche — PCA(5)+MI(1)", fontsize=14, fontweight="bold")
plt.tight_layout()
...
# 10d. Decision boundaries (PC1 vs PC2)
# Per visualizzazione 2D mostriamo solo i primi due assi del vettore finale.
# Nota importante: per la QSVM questa è solo una approssimazione visiva, perché
# il vero classificatore lavora nello spazio completo a 6 dimensioni e usa il
# kernel quantistico precomputed. Qui, per ottenere un contorno 2D interpretabile,
# usiamo una approssimazione con kernel RBF sul piano (PC1, PC2).
print("[PLOT] Generazione decision boundaries 2D (PC1 vs PC2)")

def plot_decision_boundary(clf, X, y, model_name, ax, kernel_mode=False):
    """
    Disegna la frontiera decisionale nel piano delle prime due componenti.
    - Se kernel_mode=False, clf lavora direttamente sul piano 2D
    - Se kernel_mode=True, la predizione sulla griglia usa una matrice kernel
      costruita rispetto ai punti di training 2D
    """
    h = 0.05
    ...
    ax.set_xlabel("PC1 [rad]")
    ax.set_ylabel("PC2 [rad]")
    ax.set_title(model_name, fontsize=12, fontweight="bold")
    ax.legend(fontsize=9)

# Consideriamo solo le prime due dimensioni per la visualizzazione
X_2d_train = X_train[:, :2]
X_2d_test = X_test[:, :2]
print(f"[INFO] Spazio 2D per decision boundaries: {X_2d_train.shape}")

# SVM lineare nel piano 2D
svm_lin_2d = SVC(kernel="linear", C=1.0, random_state=42)
svm_lin_2d.fit(X_2d_train, y_train)
print("[FIT] svm_lin_2d addestrata")

# SVM RBF nel piano 2D
svm_rbf_2d = SVC(kernel="rbf", C=1.0, gamma="scale", random_state=42)
svm_rbf_2d.fit(X_2d_train, y_train)
print("[FIT] svm_rbf_2d addestrata")

# QSVM approx 2D per visualizzazione
# Usiamo una SVC precomputed alimentata da un kernel RBF 2D solo per tracciare
# una frontiera comparabile sul piano. Non rappresenta il kernel quantistico reale
# a 6 dimensioni, ma è utile come illustrazione qualitativa.
qsvm_2d = SVC(kernel="precomputed", C=1.0, random_state=42)
from sklearn.metrics.pairwise import rbf_kernel
K_2d = rbf_kernel(X_2d_train)
qsvm_2d.fit(K_2d, y_train)
print("[FIT] qsvm_2d approx addestrata (kernel RBF su piano 2D)")
...
plot_decision_boundary(qsvm_2d, X_2d_train, y_train, "QSVM (approx 2D)", axes[2], kernel_mode=True)
fig.suptitle("Decision Boundaries — PC1 vs PC2 (Training Set, setup 6 qubit)",
             fontsize=14, fontweight="bold")
plt.tight_layout()
plt.savefig("decision_boundaries_6q.png", dpi=150)
plt.close()
print("[PLOT] Salvato decision_boundaries_6q.png")
...
with open("readme_6q.txt", "w", encoding="utf-8") as f:
    f.write(
        ...
        "Nota: il passaggio da PCA(4) a PCA(5) aumenta la varianza spiegata.\n"
            "Kernel: Projected Quantum Kernel (PQK) — expectation values locali <Z_i> + RBF.\n"
...
print("\n" + "=" * 84)
print("PIPELINE 6 QUBIT PQK COMPLETATA")
print("=" * 84)
